...
def graphic_donut_custom():
    ...
    field_names = ["Headline (optional)", "Percent (0-100)", "Description (Text)", "Label (Text)", "Color (Red, 0-255)", "Color (Green, 0-255)", "Color (Blue, 0-255)", "Clockwise (y/n)"]
    field_values = gui.multenterbox(msg, title, field_names)
    if field_values == None:
        return
    ...
    percent_value = float(field_values[1].replace('%', ''))
    description = field_values[2]
    percent_label = field_values[3] if field_values[3] != '' else str(percent_value)
    ...
    clockwise = not (field_values[7] in ["n", "N", "no", "No", "0"])
    
    img = osf.graphics.donut(percent_value, {"label":percent_label, "description":description, "color":(color_red, color_green, color_blue), "clockwise":clockwise, "headline":headline})
    img.show()
